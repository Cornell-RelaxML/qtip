...
def generate(model, tokenizer, text, max_new_tokens, top_k, callback, past_kv):
    ...
    for _ in range(1, max_new_tokens):
        with torch.backends.cuda.sdp_kernel(enable_flash=True, enable_mem_efficient=False, enable_math=True):
            next_token, logits = decode_one_tokens(model, next_token.clone(), past_kv, cache_position)
...
def main(hf_path, compile, interactive, num_samples, max_tokens, top_k):
    ...
    past_kv = StaticCache(model.config, 1, 2048, device=model.device, dtype=model.dtype)
    ...
    while True:
        ...
        if not interactive:
            callback = lambda x : x
        ids, text, decode_tps = generate(model, tokenizer, text, max_tokens, top_k, callback, past_kv)
        if not interactive:
            print(text)
        else:
            print()
